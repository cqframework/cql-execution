...
abstract class Memoizer<
  S,
  S_KEY,
  P extends KeyValuePair<S_KEY, S>,
  T,
  T_KEY,
 ...
 * 4. memoize
 */
abstract class InMemoryCacheMemoizer<
  S,
  S_KEY,
  P extends KeyValuePair<S_KEY, S>,
  T,
 ...
 */
export class ImmutableMemoizer<S, T> extends InMemoryCacheMemoizer<
  S,
  ImmutableObjectKey,
  ImmutableKeyValuePair<S>,
  T,
  ImmutableObjectKey,
  ImmutableKeyValuePair<T>,
  ImmutableListKey
> {
  protected readonly cache = Immutable.Map<
    ImmutableListKey,
    ImmutableKeyValuePair<T>[]
  >().asMutable();
  ...
  protected getListCacheKey = (...list: ImmutableKeyValuePair<S>[][]) =>
    Immutable.List(list.map(x => Immutable.List(x.map(y => y.key))));
    ...
    switch (js.constructor) {
      ...
      default:
        ...
        return Immutable.Seq(js)
          .map((x: any) => this.toImmutableObjectKey(x))
          .toMap()
          .set('__instance', this.toImmutableObjectKey(js._typeHierarchy?.()) ?? js.constructor);
    }
  };
}
