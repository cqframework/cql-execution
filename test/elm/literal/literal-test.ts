...
describe('Literal', () => {
  ...
  it('should convert .1 to decimal .1', function () {
    this.decimalTenth.value.should.eql(Decimal.from(0.1));
  });

  it('should execute .1 as .1', async function () {
    (await this.decimalTenth.exec(this.ctx)).should.eql(Decimal.from(0.1));
  });
  ...
  it("should execute '' as correct DateTime", async function () {
    ...
    d.timezoneOffset.should.eql(Decimal.from(0));
  });
