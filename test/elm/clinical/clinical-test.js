...
describe('InValueSet', function() {
  ...
  it('should throw an error when codes are in several codesystems', function() {
    should(() => this.sharedCodesFoo.exec(this.ctx)).throw('In (valueset) is ambiguous -- multiple codes with multiple code systems exist in value set.');
  });
  ...
  it('should throw an error if not all codes have the same codesystem', function() {
    should(() => this.improperSharedCodesCodeValue.exec(this.ctx)).throw('In (valueset) is ambiguous -- multiple codes with multiple code systems exist in value set.');
  });
...
describe('CalculateAge', function() {
  ...
  it.skip('should execute age in weeks', function() {
    ...
    this.weeks.exec(this.ctx).should.eql(Math.floor(Math.floor(Math.floor(Math.floor(Math.floor(Math.floor(this.timediff / 1000) / 60) / 60) / 24) / 7)));
  });
