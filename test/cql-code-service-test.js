...
describe('CodeService', function() {
  this.beforeEach(function() {
    ...
    this.vsThree = new ValueSet('6.7.8.9.0', 'A', [
  ...
  it('should find value sets by OID', function() {
    ...
    valueSets.should.containEql(this.vsThree);
  });

  it('should find a single value set by OID and version', function() {
    this.svc.findValueSet('1.2.3.4.5', '1').should.eql(this.vsOne);
  });

  it('should find a single value set by OID (using latest version)', function() {
    this.svc.findValueSet('1.2.3.4.5').should.eql(this.vsTwo);
  });

  it('should return empty array when searching for value sets by wrong OID', function() {
    this.svc.findValueSetsByOid('0.0.0.0.0').should.be.empty();
  });

  it('should return null when looking for a single value set by wrong OID', function() {
    should.not.exist(this.svc.findValueSet('0.0.0.0.0'));
  });

  it('should return null when looking for a single value set by wrong version', function() {
    should.not.exist(this.svc.findValueSet('1.2.3.4.5', '3'));
  });
});
